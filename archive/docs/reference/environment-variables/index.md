...
## Provider Auth (OAuth)[​](index.md#provider-auth-oauth "Direct link to Provider Auth (OAuth)")
...
## Tool APIs[​](index.md#tool-apis "Direct link to Tool APIs")
...
### Langfuse Observability[​](index.md#langfuse-observability "Direct link to Langfuse Observability")

Environment variables for the bundled [`observability/langfuse`](../../user-guide/features/built-in-plugins/index.md#observabilitylangfuse) plugin. Set these with `hermes tools → Langfuse Observability` or manually in `~/.hermes/.env`. The plugin must also be enabled (`hermes plugins enable observability/langfuse`) before any of these take effect.
...
### Nous Tool Gateway[​](index.md#nous-tool-gateway "Direct link to Nous Tool Gateway")

These variables configure the [Tool Gateway](../../user-guide/features/tool-gateway/index.md) for paid Nous subscribers or self-hosted gateway deployments. Most users don't need to set these — the gateway is configured automatically via `hermes model` or `hermes tools`.
...
## Terminal Backend[​](index.md#terminal-backend "Direct link to Terminal Backend")
...
## Container Resources (Docker, Singularity, Modal, Daytona)[​](index.md#container-resources-docker-singularity-modal-daytona "Direct link to Container Resources (Docker, Singularity, Modal, Daytona)")
...
## Messaging[​](index.md#messaging "Direct link to Messaging")
...
| `DISCORD_ALLOW_MENTION_EVERYONE` | Allow the bot to ping `@everyone`/`@here` (default: `false`). See [Mention Control](../../user-guide/messaging/discord/index.md#mention-control). |
...
| `FEISHU_ALLOW_BOTS` | `none` (default) / `mentions` / `all` — accept inbound messages from other bots. See [bot-to-bot messaging](../../user-guide/messaging/feishu/index.md#bot-to-bot-messaging) |
...
| `GATEWAY_PROXY_URL` | URL of a remote Hermes API server to forward messages to ([proxy mode](../../user-guide/messaging/matrix/index.md#proxy-mode-e2ee-on-macos)). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Also configurable via `gateway.proxy_url` in `config.yaml`. |
...
### Microsoft Graph (Teams Meetings)[​](index.md#microsoft-graph-teams-meetings "Direct link to Microsoft Graph (Teams Meetings)")

App-only credentials for the Microsoft Graph REST client used by the upcoming Teams meeting summary pipeline. See [Register a Microsoft Graph application](../../guides/microsoft-graph-app-registration/index.md) for the Azure portal walkthrough and the exact API permissions required.
...
### Microsoft Graph Webhook Listener[​](index.md#microsoft-graph-webhook-listener "Direct link to Microsoft Graph Webhook Listener")

Inbound change-notification listener for Graph events (Teams meetings, calendar, chat, etc.). See [Microsoft Graph Webhook Listener](../../user-guide/messaging/msgraph-webhook/index.md) for setup and security hardening.
...
### Teams Meeting Summary Delivery[​](index.md#teams-meeting-summary-delivery "Direct link to Teams Meeting Summary Delivery")

Only used when the [`teams_pipeline` plugin](../../user-guide/messaging/msgraph-webhook/index.md) is enabled. Settings are also configurable under `platforms.teams.extra` in `config.yaml` — env vars take priority when both are set. See [Microsoft Teams → Meeting Summary Delivery](../../user-guide/messaging/teams/index.md#meeting-summary-delivery-teams-meeting-pipeline).
...
### LINE Messaging API[​](index.md#line-messaging-api "Direct link to LINE Messaging API")

Used by the bundled LINE platform plugin (`plugins/platforms/line/`). See [Messaging Gateway → LINE](../../user-guide/messaging/line/index.md) for full setup.
...
### Advanced Messaging Tuning[​](index.md#advanced-messaging-tuning "Direct link to Advanced Messaging Tuning")
...
## Agent Behavior[​](index.md#agent-behavior "Direct link to Agent Behavior")
...
## Interface[​](index.md#interface "Direct link to Interface")

| Variable | Description |
| --- | --- |
| `HERMES_TUI` | Launch the [TUI](../../user-guide/tui/index.md) instead of the classic CLI when set to `1`. Equivalent to passing `--tui`. |
...
## Session Settings[​](index.md#session-settings "Direct link to Session Settings")
...
## Context Compression (config.yaml only)[​](index.md#context-compression-configyaml-only "Direct link to Context Compression (config.yaml only)")
...
## Auxiliary Task Overrides[​](index.md#auxiliary-task-overrides "Direct link to Auxiliary Task Overrides")
...
## Fallback Providers (config.yaml only)[​](index.md#fallback-providers-configyaml-only "Direct link to Fallback Providers (config.yaml only)")
...
See [Fallback Providers](../../user-guide/features/fallback-providers/index.md) for full details.

## Provider Routing (config.yaml only)[​](index.md#provider-routing-configyaml-only "Direct link to Provider Routing (config.yaml only)")
...
- [LLM Providers](index.md#llm-providers)
- [Provider Auth (OAuth)](index.md#provider-auth-oauth)
- [Tool APIs](index.md#tool-apis)
  - [Langfuse Observability](index.md#langfuse-observability)
  - [Nous Tool Gateway](index.md#nous-tool-gateway)
- [Terminal Backend](index.md#terminal-backend)
- [SSH Backend](index.md#ssh-backend)
- [Container Resources (Docker, Singularity, Modal, Daytona)](index.md#container-resources-docker-singularity-modal-daytona)
- [Persistent Shell](index.md#persistent-shell)
- [Messaging](index.md#messaging)
  - [Microsoft Graph (Teams Meetings)](index.md#microsoft-graph-teams-meetings)
  - [Microsoft Graph Webhook Listener](index.md#microsoft-graph-webhook-listener)
  - [Teams Meeting Summary Delivery](index.md#teams-meeting-summary-delivery)
  - [LINE Messaging API](index.md#line-messaging-api)
  - [Advanced Messaging Tuning](index.md#advanced-messaging-tuning)
- [Agent Behavior](index.md#agent-behavior)
- [Interface](index.md#interface)
- [Session Settings](index.md#session-settings)
- [Context Compression (config.yaml only)](index.md#context-compression-configyaml-only)
- [Auxiliary Task Overrides](index.md#auxiliary-task-overrides)
- [Fallback Providers (config.yaml only)](index.md#fallback-providers-configyaml-only)
- [Provider Routing (config.yaml only)](index.md#provider-routing-configyaml-only)
