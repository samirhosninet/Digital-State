...
title: Slash Commands Reference | Hermes Agent
...
## Interactive CLI slash commands[​](index.md#interactive-cli-slash-commands "Direct link to Interactive CLI slash commands")

Type `/` in the CLI to open the autocomplete menu. Built-in commands are case-insensitive.

### Session[​](index.md#session "Direct link to Session")
...
| `/background <prompt>` (alias: `/bg`, `/btw`) | Run a prompt in a separate background session. The agent processes your prompt independently — your current session stays free for other work. Results appear as a panel when the task finishes. See [CLI Background Sessions](../../user-guide/cli/index.md#background-sessions). |
| `/branch [name]` (alias: `/fork`) | Branch the current session (explore a different path) |
| `/handoff <platform>` | **CLI only.** Hand the current session off to a messaging platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix). The gateway picks it up immediately, creates a fresh thread on platforms that support threads (Telegram topics, Discord text-channel threads, Slack message-anchored threads), re-binds the destination to your CLI session\_id so the full role-aware transcript replays, and forges a synthetic user turn so the agent confirms it's working in the new place. Your CLI exits cleanly on success with a `/resume` hint; resume locally any time with `/resume <title>`. Refused mid-turn. Requires the gateway to be running and a home channel configured for the target platform (`/sethome` from the destination chat). See [Cross-Platform Handoff](../../user-guide/sessions/index.md#cross-platform-handoff). |
...
| `/model [model-name]` | Show or change the current model. Supports: `/model claude-sonnet-4`, `/model provider:model` (switch providers), `/model custom:model` (custom endpoint), `/model custom:name:model` (named custom provider), `/model custom` (auto-detect from endpoint), and user-defined aliases (`/model fav`, `/model grok` — see [Custom model aliases](index.md#custom-model-aliases)). Use `--global` to persist the change to config.yaml. **Note:** `/model` can only switch between already-configured providers. To add a new provider, exit the session and run `hermes model` from your terminal. |
| `/personality` | Set a predefined personality |
| `/verbose` | Cycle tool progress display: off → new → all → verbose. Can be [enabled for messaging](index.md#notes) via config. |
...
### Tools & Skills[​](index.md#tools--skills "Direct link to Tools & Skills")
...
### Info[​](index.md#info "Direct link to Info")
...
### Exit[​](index.md#exit "Direct link to Exit")

| Command | Description |
| --- | --- |
| `/quit` | Exit the CLI (also: `/exit`). |

### Dynamic CLI slash commands[​](index.md#dynamic-cli-slash-commands "Direct link to Dynamic CLI slash commands")
...
### Custom model aliases[​](index.md#custom-model-aliases "Direct link to Custom model aliases")
...
### Alias Resolution[​](index.md#alias-resolution "Direct link to Alias Resolution")
...
## Messaging slash commands[​](index.md#messaging-slash-commands "Direct link to Messaging slash commands")
...
| `/model [provider:model]` | Show or change the model. Supports provider switches (`/model zai:glm-5`), custom endpoints (`/model custom:model`), named custom providers (`/model custom:local:qwen`), auto-detect (`/model custom`), and user-defined aliases (`/model fav`, `/model grok` — see [Custom model aliases](index.md#custom-model-aliases)). Use `--global` to persist the change to config.yaml. **Note:** `/model` can only switch between already-configured providers. To add a new provider or set up API keys, use `hermes model` from your terminal (outside the chat session). |
...
| `/topic [off|help|session-id]` | **Telegram DM only.** Manage user-managed multi-session topic mode. `/topic` enables it or shows status; `/topic off` disables it and clears bindings; `/topic help` shows usage; `/topic <session-id>` inside a topic restores a previous session. See [Multi-session DM mode](../../user-guide/messaging/telegram/index.md#multi-session-dm-mode-topic). |
...
| `/background <prompt>` | Run a prompt in a separate background session. Results are delivered back to the same chat when the task finishes. See [Messaging Background Sessions](../../user-guide/messaging/index.md#background-sessions). |
...
- [Interactive CLI slash commands](index.md#interactive-cli-slash-commands)
  - [Session](index.md#session)
  - [Configuration](index.md#configuration)
  - [Tools & Skills](index.md#tools--skills)
  - [Info](index.md#info)
  - [Exit](index.md#exit)
  - [Dynamic CLI slash commands](index.md#dynamic-cli-slash-commands)
  - [Quick Commands](index.md#quick-commands)
  - [Custom model aliases](index.md#custom-model-aliases)
  - [Alias Resolution](index.md#alias-resolution)
- [Messaging slash commands](index.md#messaging-slash-commands)
