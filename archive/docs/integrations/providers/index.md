...
title: AI Providers | Hermes Agent
...
## Inference Providers[​](index.md#inference-providers "Direct link to Inference Providers")
...
For the official API-key path, see the dedicated [Google Gemini guide](../../guides/google-gemini/index.md).

Model key alias

In the `model:` config section, you can use either `default:` or `model:` as the key name for your model ID. Both `model: { default: my-model }` and `model: { model: my-model }` work identically.

### Google Gemini via OAuth (`google-gemini-cli`)[​](index.md#google-gemini-via-oauth-google-gemini-cli "Direct link to google-gemini-via-oauth-google-gemini-cli")
...
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Hermes routes these tasks to your **main chat model** — the same model you picked in `hermes model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](../../user-guide/configuration/index.md#auxiliary-models).

Nous Tool Gateway

Paid Nous Portal subscribers also get access to the **[Tool Gateway](../../user-guide/features/tool-gateway/index.md)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. It's offered automatically during `hermes model` setup, or enable it later with `hermes tools`.

### Two Commands for Model Management[​](index.md#two-commands-for-model-management "Direct link to Two Commands for Model Management")
...
### Anthropic (Native)[​](index.md#anthropic-native "Direct link to Anthropic (Native)")
...
### GitHub Copilot[​](index.md#github-copilot "Direct link to GitHub Copilot")
...
### First-Class API-Key Providers[​](index.md#first-class-api-key-providers "Direct link to First-Class API-Key Providers")
...
### xAI (Grok) — Responses API + Prompt Caching[​](index.md#xai-grok--responses-api--prompt-caching "Direct link to xAI (Grok) — Responses API + Prompt Caching")
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `hermes tools` → Voice & TTS, or see the [Voice & TTS](../../user-guide/features/tts/index.md#text-to-speech) page for config.

### Ollama Cloud — Managed Ollama Models, OAuth + API Key[​](index.md#ollama-cloud--managed-ollama-models-oauth--api-key "Direct link to Ollama Cloud — Managed Ollama Models, OAuth + API Key")
...
### AWS Bedrock[​](index.md#aws-bedrock "Direct link to AWS Bedrock")
...
See the [AWS Bedrock guide](../../guides/aws-bedrock/index.md) for a walkthrough of IAM setup, region selection, and cross-region inference.

### Qwen Portal (OAuth)[​](index.md#qwen-portal-oauth "Direct link to Qwen Portal (OAuth)")
...
### Alibaba Coding Plan[​](index.md#alibaba-coding-plan "Direct link to Alibaba Coding Plan")
...
### MiniMax (OAuth)[​](index.md#minimax-oauth "Direct link to MiniMax (OAuth)")
...
`minimax-oauth` uses MiniMax's consumer-facing portal with OAuth login — no billing setup required. The `minimax` and `minimax-cn` providers use `MINIMAX_API_KEY` / `MINIMAX_CN_API_KEY` — for programmatic access. See the [MiniMax OAuth guide](../../guides/minimax-oauth/index.md) for a full walkthrough.

### NVIDIA NIM[​](index.md#nvidia-nim "Direct link to NVIDIA NIM")
...
### GMI Cloud[​](index.md#gmi-cloud "Direct link to GMI Cloud")
...
### StepFun[​](index.md#stepfun "Direct link to StepFun")
...
### Hugging Face Inference Providers[​](index.md#hugging-face-inference-providers "Direct link to Hugging Face Inference Providers")
...
## Custom & Self-Hosted LLM Providers[​](index.md#custom--self-hosted-llm-providers "Direct link to Custom & Self-Hosted LLM Providers")

Hermes Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Hermes at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.

### General Setup[​](index.md#general-setup "Direct link to General Setup")
...
### Switching Models with `/model`[​](index.md#switching-models-with-model "Direct link to switching-models-with-model")
...
### Ollama — Local Models, Zero Config[​](index.md#ollama--local-models-zero-config "Direct link to Ollama — Local Models, Zero Config")
...
### vLLM — High-Performance GPU Inference[​](index.md#vllm--high-performance-gpu-inference "Direct link to vLLM — High-Performance GPU Inference")
...
### SGLang — Fast Serving with RadixAttention[​](index.md#sglang--fast-serving-with-radixattention "Direct link to SGLang — Fast Serving with RadixAttention")
...
### llama.cpp / llama-server — CPU & Metal Inference[​](index.md#llamacpp--llama-server--cpu--metal-inference "Direct link to llama.cpp / llama-server — CPU & Metal Inference")
...
### LM Studio — Desktop App with Local Models[​](index.md#lm-studio--desktop-app-with-local-models "Direct link to LM Studio — Desktop App with Local Models")
...
### WSL2 Networking (Windows Users)[​](index.md#wsl2-networking-windows-users "Direct link to WSL2 Networking (Windows Users)")
...
#### Option 1: Mirrored Networking Mode (Recommended)[​](index.md#option-1-mirrored-networking-mode-recommended "Direct link to Option 1: Mirrored Networking Mode (Recommended)")
...
#### Option 2: Use the Windows Host IP (Windows 10 / older builds)[​](index.md#option-2-use-the-windows-host-ip-windows-10--older-builds "Direct link to Option 2: Use the Windows Host IP (Windows 10 / older builds)")
...
#### Server Bind Address (Required for NAT Mode)[​](index.md#server-bind-address-required-for-nat-mode "Direct link to Server Bind Address (Required for NAT Mode)")
...
#### Windows Firewall[​](index.md#windows-firewall "Direct link to Windows Firewall")
...
#### Quick Verification[​](index.md#quick-verification "Direct link to Quick Verification")
...
### Troubleshooting Local Models[​](index.md#troubleshooting-local-models "Direct link to Troubleshooting Local Models")

These issues affect **all** local inference servers when used with Hermes.

#### "Connection refused" from WSL2 to a Windows-hosted model server[​](index.md#connection-refused-from-wsl2-to-a-windows-hosted-model-server "Direct link to \"Connection refused\" from WSL2 to a Windows-hosted model server")

If you're running Hermes inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](index.md#wsl2-networking-windows-users) above for the fix.

#### Tool calls appear as text instead of executing[​](index.md#tool-calls-appear-as-text-instead-of-executing "Direct link to Tool calls appear as text instead of executing")
...
#### Model seems to forget context or give incoherent responses[​](index.md#model-seems-to-forget-context-or-give-incoherent-responses "Direct link to Model seems to forget context or give incoherent responses")
...
#### "Context limit: 2048 tokens" at startup[​](index.md#context-limit-2048-tokens-at-startup "Direct link to \"Context limit: 2048 tokens\" at startup")
...
#### Responses get cut off mid-sentence[​](index.md#responses-get-cut-off-mid-sentence "Direct link to Responses get cut off mid-sentence")
...
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](../../user-guide/configuration/index.md#context-compression) in Hermes.

---

### LiteLLM Proxy — Multi-Provider Gateway[​](index.md#litellm-proxy--multi-provider-gateway "Direct link to LiteLLM Proxy — Multi-Provider Gateway")
...
### ClawRouter — Cost-Optimized Routing[​](index.md#clawrouter--cost-optimized-routing "Direct link to ClawRouter — Cost-Optimized Routing")
...
### Other Compatible Providers[​](index.md#other-compatible-providers "Direct link to Other Compatible Providers")
...
### Context Length Detection[​](index.md#context-length-detection "Direct link to Context Length Detection")
...
### Named Custom Providers[​](index.md#named-custom-providers "Direct link to Named Custom Providers")
...
### Cookbook: Together AI, Groq, Perplexity[​](index.md#cookbook-together-ai-groq-perplexity "Direct link to Cookbook: Together AI, Groq, Perplexity")

The cloud providers listed in [Other Compatible Providers](index.md#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.hermes/config.yaml` and the matching API key goes in `~/.hermes/.env`.

#### Together AI[​](index.md#together-ai "Direct link to Together AI")
...
#### Groq[​](index.md#groq "Direct link to Groq")
...
#### Perplexity[​](index.md#perplexity "Direct link to Perplexity")
...
#### Multiple providers in one config[​](index.md#multiple-providers-in-one-config "Direct link to Multiple providers in one config")
...
### Choosing the Right Setup[​](index.md#choosing-the-right-setup "Direct link to Choosing the Right Setup")
...
## Optional API Keys[​](index.md#optional-api-keys "Direct link to Optional API Keys")
...
### Self-Hosting Firecrawl[​](index.md#self-hosting-firecrawl "Direct link to Self-Hosting Firecrawl")
...
## OpenRouter Provider Routing[​](index.md#openrouter-provider-routing "Direct link to OpenRouter Provider Routing")
...
## OpenRouter Pareto Code Router[​](index.md#openrouter-pareto-code-router "Direct link to OpenRouter Pareto Code Router")
...
- To use the Pareto Code router for a specific **auxiliary task** (compression, vision, etc.) instead of the main agent, set `extra_body.plugins` under that task — see [Auxiliary Models → OpenRouter routing & Pareto Code for auxiliary tasks](../../user-guide/configuration/index.md#openrouter-routing--pareto-code-for-auxiliary-tasks).

## Fallback Providers[​](index.md#fallback-providers "Direct link to Fallback Providers")
...
Fallback is configured exclusively through `config.yaml` — or interactively via `hermes fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](../../user-guide/features/fallback-providers/index.md).

---

## See Also[​](index.md#see-also "Direct link to See Also")

- [Configuration](../../user-guide/configuration/index.md) — General configuration (directory structure, config precedence, terminal backends, memory, compression, and more)
- [Environment Variables](../../reference/environment-variables/index.md) — Complete reference of all environment variables

- [Inference Providers](index.md#inference-providers)
  - [Google Gemini via OAuth (`google-gemini-cli`)](index.md#google-gemini-via-oauth-google-gemini-cli)
  - [Two Commands for Model Management](index.md#two-commands-for-model-management)
  - [Anthropic (Native)](index.md#anthropic-native)
  - [GitHub Copilot](index.md#github-copilot)
  - [First-Class API-Key Providers](index.md#first-class-api-key-providers)
  - [xAI (Grok) — Responses API + Prompt Caching](index.md#xai-grok--responses-api--prompt-caching)
  - [Ollama Cloud — Managed Ollama Models, OAuth + API Key](index.md#ollama-cloud--managed-ollama-models-oauth--api-key)
  - [AWS Bedrock](index.md#aws-bedrock)
  - [Qwen Portal (OAuth)](index.md#qwen-portal-oauth)
  - [Alibaba Coding Plan](index.md#alibaba-coding-plan)
  - [MiniMax (OAuth)](index.md#minimax-oauth)
  - [NVIDIA NIM](index.md#nvidia-nim)
  - [GMI Cloud](index.md#gmi-cloud)
  - [StepFun](index.md#stepfun)
  - [Hugging Face Inference Providers](index.md#hugging-face-inference-providers)
- [Custom & Self-Hosted LLM Providers](index.md#custom--self-hosted-llm-providers)
  - [General Setup](index.md#general-setup)
  - [Switching Models with `/model`](index.md#switching-models-with-model)
  - [Ollama — Local Models, Zero Config](index.md#ollama--local-models-zero-config)
  - [vLLM — High-Performance GPU Inference](index.md#vllm--high-performance-gpu-inference)
  - [SGLang — Fast Serving with RadixAttention](index.md#sglang--fast-serving-with-radixattention)
  - [llama.cpp / llama-server — CPU & Metal Inference](index.md#llamacpp--llama-server--cpu--metal-inference)
  - [LM Studio — Desktop App with Local Models](index.md#lm-studio--desktop-app-with-local-models)
  - [WSL2 Networking (Windows Users)](index.md#wsl2-networking-windows-users)
  - [Troubleshooting Local Models](index.md#troubleshooting-local-models)
  - [LiteLLM Proxy — Multi-Provider Gateway](index.md#litellm-proxy--multi-provider-gateway)
  - [ClawRouter — Cost-Optimized Routing](index.md#clawrouter--cost-optimized-routing)
  - [Other Compatible Providers](index.md#other-compatible-providers)
  - [Context Length Detection](index.md#context-length-detection)
  - [Named Custom Providers](index.md#named-custom-providers)
  - [Cookbook: Together AI, Groq, Perplexity](index.md#cookbook-together-ai-groq-perplexity)
  - [Choosing the Right Setup](index.md#choosing-the-right-setup)
- [Optional API Keys](index.md#optional-api-keys)
  - [Self-Hosting Firecrawl](index.md#self-hosting-firecrawl)
- [OpenRouter Provider Routing](index.md#openrouter-provider-routing)
- [OpenRouter Pareto Code Router](index.md#openrouter-pareto-code-router)
- [Fallback Providers](index.md#fallback-providers)
