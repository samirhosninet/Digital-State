...
title: Adding a Platform Adapter | Hermes Agent
...
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.hermes/plugins/` — zero core code changes needed. See [Plugin Path](index.md#plugin-path-recommended) below.
- **Built-in**: Modify 20+ files across code, config, and docs. Use the [Built-in Checklist](index.md#step-by-step-checklist) below.
...
## Plugin Path (Recommended)[​](index.md#plugin-path-recommended "Direct link to Plugin Path (Recommended)")
...
### PLUGIN.yaml[​](index.md#pluginyaml "Direct link to PLUGIN.yaml")

Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `hermes config` UI entries (see [Surfacing Env Vars](index.md#surfacing-env-vars-in-hermes-config) below).
...
### adapter.py[​](index.md#adapterpy "Direct link to adapter.py")
...
### What the Plugin System Handles Automatically[​](index.md#what-the-plugin-system-handles-automatically "Direct link to What the Plugin System Handles Automatically")
...
| send\_message tool | Routes through live gateway adapter |
...
## Env-Driven Auto-Configuration[​](index.md#env-driven-auto-configuration "Direct link to Env-Driven Auto-Configuration")
...
## Cron Delivery[​](index.md#cron-delivery "Direct link to Cron Delivery")
...
### Out-of-process cron delivery[​](index.md#out-of-process-cron-delivery "Direct link to Out-of-process cron delivery")
...
## Surfacing Env Vars in `hermes config`[​](index.md#surfacing-env-vars-in-hermes-config "Direct link to surfacing-env-vars-in-hermes-config")
...
## Platform-Specific Slow-LLM UX[​](index.md#platform-specific-slow-llm-ux "Direct link to Platform-Specific Slow-LLM UX")
...
### Pattern: subclass `_keep_typing` to layer mid-flight UX[​](index.md#pattern-subclass-_keep_typing-to-layer-mid-flight-ux "Direct link to pattern-subclass-_keep_typing-to-layer-mid-flight-ux")
...
### Pattern: subclass `send` to route through a cache instead of sending immediately[​](index.md#pattern-subclass-send-to-route-through-a-cache-instead-of-sending-immediately "Direct link to pattern-subclass-send-to-route-through-a-cache-instead-of-sending-immediately")
...
1. **Pending postback active for this chat** → cache the response under the request\_id, don't send anything visible.
...
### When this pattern is appropriate[​](index.md#when-this-pattern-is-appropriate "Direct link to When this pattern is appropriate")
...
### Reference Implementation[​](index.md#reference-implementation "Direct link to Reference Implementation")
...
### Reference Implementations (Plugin Path)[​](index.md#reference-implementations-plugin-path "Direct link to Reference Implementations (Plugin Path)")
...
## Step-by-Step Checklist (Built-in Path)[​](index.md#step-by-step-checklist-built-in-path "Direct link to Step-by-Step Checklist (Built-in Path)")

note

This checklist is for adding a platform directly to the Hermes core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](index.md#plugin-path-recommended) above.

### 1. Platform Enum[​](index.md#1-platform-enum "Direct link to 1. Platform Enum")
...
### 2. Adapter File[​](index.md#2-adapter-file "Direct link to 2. Adapter File")
...
### 3. Gateway Config (`gateway/config.py`)[​](index.md#3-gateway-config-gatewayconfigpy "Direct link to 3-gateway-config-gatewayconfigpy")
...
### 4. Gateway Runner (`gateway/run.py`)[​](index.md#4-gateway-runner-gatewayrunpy "Direct link to 4-gateway-runner-gatewayrunpy")
...
### 5. Cross-Platform Delivery[​](index.md#5-cross-platform-delivery "Direct link to 5. Cross-Platform Delivery")
...
### 6. CLI Integration[​](index.md#6-cli-integration "Direct link to 6. CLI Integration")

1. **`hermes_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`hermes_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token\_var, setup\_instructions, and vars
3. **`hermes_cli/platforms.py`** — Add `PlatformInfo` entry with label and default\_toolset (used by `skills_config` and `tools_config` TUIs)
...
### 7. Tools[​](index.md#7-tools "Direct link to 7. Tools")
...
### 8. Toolsets[​](index.md#8-toolsets "Direct link to 8. Toolsets")
...
### 9. Optional: Platform Hints[​](index.md#9-optional-platform-hints "Direct link to 9. Optional: Platform Hints")
...
### 10. Tests[​](index.md#10-tests "Direct link to 10. Tests")
...
### 11. Documentation[​](index.md#11-documentation "Direct link to 11. Documentation")
...
## Parity Audit[​](index.md#parity-audit "Direct link to Parity Audit")
...
### Long-Poll Adapters[​](index.md#long-poll-adapters "Direct link to Long-Poll Adapters")
...
### Callback/Webhook Adapters[​](index.md#callbackwebhook-adapters "Direct link to Callback/Webhook Adapters")
...
## Reference Implementations[​](index.md#reference-implementations "Direct link to Reference Implementations")
...
- [Plugin Path (Recommended)](index.md#plugin-path-recommended)
  - [PLUGIN.yaml](index.md#pluginyaml)
  - [adapter.py](index.md#adapterpy)
  - [Configuration](index.md#configuration)
  - [What the Plugin System Handles Automatically](index.md#what-the-plugin-system-handles-automatically)
- [Env-Driven Auto-Configuration](index.md#env-driven-auto-configuration)
- [Cron Delivery](index.md#cron-delivery)
  - [Out-of-process cron delivery](index.md#out-of-process-cron-delivery)
- [Surfacing Env Vars in `hermes config`](index.md#surfacing-env-vars-in-hermes-config)
- [Platform-Specific Slow-LLM UX](index.md#platform-specific-slow-llm-ux)
  - [Pattern: subclass `_keep_typing` to layer mid-flight UX](index.md#pattern-subclass-_keep_typing-to-layer-mid-flight-ux)
  - [Pattern: subclass `send` to route through a cache instead of sending immediately](index.md#pattern-subclass-send-to-route-through-a-cache-instead-of-sending-immediately)
  - [When this pattern is appropriate](index.md#when-this-pattern-is-appropriate)
  - [Reference Implementation](index.md#reference-implementation)
  - [Reference Implementations (Plugin Path)](index.md#reference-implementations-plugin-path)
- [Step-by-Step Checklist (Built-in Path)](index.md#step-by-step-checklist-built-in-path)
  - [1. Platform Enum](index.md#1-platform-enum)
  - [2. Adapter File](index.md#2-adapter-file)
  - [3. Gateway Config (`gateway/config.py`)](index.md#3-gateway-config-gatewayconfigpy)
  - [4. Gateway Runner (`gateway/run.py`)](index.md#4-gateway-runner-gatewayrunpy)
  - [5. Cross-Platform Delivery](index.md#5-cross-platform-delivery)
  - [6. CLI Integration](index.md#6-cli-integration)
  - [7. Tools](index.md#7-tools)
  - [8. Toolsets](index.md#8-toolsets)
  - [9. Optional: Platform Hints](index.md#9-optional-platform-hints)
  - [10. Tests](index.md#10-tests)
  - [11. Documentation](index.md#11-documentation)
- [Parity Audit](index.md#parity-audit)
- [Common Patterns](index.md#common-patterns)
  - [Long-Poll Adapters](index.md#long-poll-adapters)
  - [Callback/Webhook Adapters](index.md#callbackwebhook-adapters)
  - [Token Locks](index.md#token-locks)
- [Reference Implementations](index.md#reference-implementations)
