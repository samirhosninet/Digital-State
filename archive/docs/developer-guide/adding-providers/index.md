...
## The mental model[​](index.md#the-mental-model "Direct link to The mental model")
...
## Choose the implementation path first[​](index.md#choose-the-implementation-path-first "Direct link to Choose the implementation path first")

### Path A — OpenAI-compatible provider[​](index.md#path-a--openai-compatible-provider "Direct link to Path A — OpenAI-compatible provider")
...
### Path B — Native provider[​](index.md#path-b--native-provider "Direct link to Path B — Native provider")
...
## File checklist[​](index.md#file-checklist "Direct link to File checklist")

### Required for every built-in provider[​](index.md#required-for-every-built-in-provider "Direct link to Required for every built-in provider")
...
### Additional for native / non-OpenAI providers[​](index.md#additional-for-native--non-openai-providers "Direct link to Additional for native / non-OpenAI providers")

10. `agent/<provider>_adapter.py`
11. `run_agent.py`
12. `pyproject.toml` if a provider SDK is required

## Fast path: Simple API-key providers[​](index.md#fast-path-simple-api-key-providers "Direct link to Fast path: Simple API-key providers")
...
See `plugins/model-providers/nvidia/` or `plugins/model-providers/gmi/` as a template, and the full [Model Provider Plugin guide](../model-provider-plugin/index.md) for field reference, hook idioms, and end-to-end examples.

## Full path: OAuth and complex providers[​](index.md#full-path-oauth-and-complex-providers "Direct link to Full path: OAuth and complex providers")
...
## Step 1: Pick one canonical provider id[​](index.md#step-1-pick-one-canonical-provider-id "Direct link to Step 1: Pick one canonical provider id")
...
## Step 2: Add auth metadata in `hermes_cli/auth.py`[​](index.md#step-2-add-auth-metadata-in-hermes_cliauthpy "Direct link to step-2-add-auth-metadata-in-hermes_cliauthpy")
...
## Step 3: Add model catalog and aliases in `hermes_cli/models.py`[​](index.md#step-3-add-model-catalog-and-aliases-in-hermes_climodelspy "Direct link to step-3-add-model-catalog-and-aliases-in-hermes_climodelspy")
...
## Step 4: Resolve runtime data in `hermes_cli/runtime_provider.py`[​](index.md#step-4-resolve-runtime-data-in-hermes_cliruntime_providerpy "Direct link to step-4-resolve-runtime-data-in-hermes_cliruntime_providerpy")
...
## Step 5: Wire the CLI in `hermes_cli/main.py`[​](index.md#step-5-wire-the-cli-in-hermes_climainpy "Direct link to step-5-wire-the-cli-in-hermes_climainpy")
...
## Step 6: Keep auxiliary calls working[​](index.md#step-6-keep-auxiliary-calls-working "Direct link to Step 6: Keep auxiliary calls working")

Two files matter here:

### `agent/auxiliary_client.py`[​](index.md#agentauxiliary_clientpy "Direct link to agentauxiliary_clientpy")
...
### `agent/model_metadata.py`[​](index.md#agentmodel_metadatapy "Direct link to agentmodel_metadatapy")

Add context lengths for the provider's models so token budgeting, compression thresholds, and limits stay sane.

## Step 7: If the provider is native, add an adapter and `run_agent.py` support[​](index.md#step-7-if-the-provider-is-native-add-an-adapter-and-run_agentpy-support "Direct link to step-7-if-the-provider-is-native-add-an-adapter-and-run_agentpy-support")
...
### New adapter file[​](index.md#new-adapter-file "Direct link to New adapter file")
...
### `run_agent.py`[​](index.md#run_agentpy "Direct link to run_agentpy")
...
### Prompt caching and provider-specific request fields[​](index.md#prompt-caching-and-provider-specific-request-fields "Direct link to Prompt caching and provider-specific request fields")
...
## Step 8: Tests[​](index.md#step-8-tests "Direct link to Step 8: Tests")
...
## Step 9: Live verification[​](index.md#step-9-live-verification "Direct link to Step 9: Live verification")
...
## Step 10: Update user-facing docs[​](index.md#step-10-update-user-facing-docs "Direct link to Step 10: Update user-facing docs")
...
## OpenAI-compatible provider checklist[​](index.md#openai-compatible-provider-checklist "Direct link to OpenAI-compatible provider checklist")
...
## Native provider checklist[​](index.md#native-provider-checklist "Direct link to Native provider checklist")
...
## Common pitfalls[​](index.md#common-pitfalls "Direct link to Common pitfalls")

### 1. Adding the provider to auth but not to model parsing[​](index.md#1-adding-the-provider-to-auth-but-not-to-model-parsing "Direct link to 1. Adding the provider to auth but not to model parsing")

That makes credentials resolve correctly while `/model` and `provider:model` inputs fail.

### 2. Forgetting that `config["model"]` can be a string or a dict[​](index.md#2-forgetting-that-configmodel-can-be-a-string-or-a-dict "Direct link to 2-forgetting-that-configmodel-can-be-a-string-or-a-dict")

A lot of provider-selection code has to normalize both forms.

### 3. Assuming a built-in provider is required[​](index.md#3-assuming-a-built-in-provider-is-required "Direct link to 3. Assuming a built-in provider is required")

If the service is just OpenAI-compatible, a custom provider may already solve the user problem with less maintenance.

### 4. Forgetting auxiliary paths[​](index.md#4-forgetting-auxiliary-paths "Direct link to 4. Forgetting auxiliary paths")
...
### 5. Native-provider branches hiding in `run_agent.py`[​](index.md#5-native-provider-branches-hiding-in-run_agentpy "Direct link to 5-native-provider-branches-hiding-in-run_agentpy")

Search for `api_mode` and `self.client.`. Do not assume the obvious request path is the only one.

### 6. Sending OpenRouter-only knobs to other providers[​](index.md#6-sending-openrouter-only-knobs-to-other-providers "Direct link to 6. Sending OpenRouter-only knobs to other providers")

Fields like provider routing belong only on the providers that support them.

### 7. Updating `hermes model` but not `hermes setup`[​](index.md#7-updating-hermes-model-but-not-hermes-setup "Direct link to 7-updating-hermes-model-but-not-hermes-setup")

Both flows need to know about the provider.

## Good search targets while implementing[​](index.md#good-search-targets-while-implementing "Direct link to Good search targets while implementing")
...
- [The mental model](index.md#the-mental-model)
- [Choose the implementation path first](index.md#choose-the-implementation-path-first)
  - [Path A — OpenAI-compatible provider](index.md#path-a--openai-compatible-provider)
  - [Path B — Native provider](index.md#path-b--native-provider)
- [File checklist](index.md#file-checklist)
  - [Required for every built-in provider](index.md#required-for-every-built-in-provider)
  - [Additional for native / non-OpenAI providers](index.md#additional-for-native--non-openai-providers)
- [Fast path: Simple API-key providers](index.md#fast-path-simple-api-key-providers)
- [Full path: OAuth and complex providers](index.md#full-path-oauth-and-complex-providers)
- [Step 1: Pick one canonical provider id](index.md#step-1-pick-one-canonical-provider-id)
- [Step 2: Add auth metadata in `hermes_cli/auth.py`](index.md#step-2-add-auth-metadata-in-hermes_cliauthpy)
- [Step 3: Add model catalog and aliases in `hermes_cli/models.py`](index.md#step-3-add-model-catalog-and-aliases-in-hermes_climodelspy)
- [Step 4: Resolve runtime data in `hermes_cli/runtime_provider.py`](index.md#step-4-resolve-runtime-data-in-hermes_cliruntime_providerpy)
- [Step 5: Wire the CLI in `hermes_cli/main.py`](index.md#step-5-wire-the-cli-in-hermes_climainpy)
- [Step 6: Keep auxiliary calls working](index.md#step-6-keep-auxiliary-calls-working)
  - [`agent/auxiliary_client.py`](index.md#agentauxiliary_clientpy)
  - [`agent/model_metadata.py`](index.md#agentmodel_metadatapy)
- [Step 7: If the provider is native, add an adapter and `run_agent.py` support](index.md#step-7-if-the-provider-is-native-add-an-adapter-and-run_agentpy-support)
  - [New adapter file](index.md#new-adapter-file)
  - [`run_agent.py`](index.md#run_agentpy)
  - [Prompt caching and provider-specific request fields](index.md#prompt-caching-and-provider-specific-request-fields)
- [Step 8: Tests](index.md#step-8-tests)
- [Step 9: Live verification](index.md#step-9-live-verification)
- [Step 10: Update user-facing docs](index.md#step-10-update-user-facing-docs)
- [OpenAI-compatible provider checklist](index.md#openai-compatible-provider-checklist)
- [Native provider checklist](index.md#native-provider-checklist)
- [Common pitfalls](index.md#common-pitfalls)
  - [1. Adding the provider to auth but not to model parsing](index.md#1-adding-the-provider-to-auth-but-not-to-model-parsing)
  - [2. Forgetting that `config["model"]` can be a string or a dict](index.md#2-forgetting-that-configmodel-can-be-a-string-or-a-dict)
  - [3. Assuming a built-in provider is required](index.md#3-assuming-a-built-in-provider-is-required)
  - [4. Forgetting auxiliary paths](index.md#4-forgetting-auxiliary-paths)
  - [5. Native-provider branches hiding in `run_agent.py`](index.md#5-native-provider-branches-hiding-in-run_agentpy)
  - [6. Sending OpenRouter-only knobs to other providers](index.md#6-sending-openrouter-only-knobs-to-other-providers)
  - [7. Updating `hermes model` but not `hermes setup`](index.md#7-updating-hermes-model-but-not-hermes-setup)
- [Good search targets while implementing](index.md#good-search-targets-while-implementing)
