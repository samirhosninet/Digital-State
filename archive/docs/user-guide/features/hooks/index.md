...
| --- | --- | --- | --- |
| **[Gateway hooks](index.md#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.hermes/hooks/` | Gateway only | Logging, alerts, webhooks |
| **[Plugin hooks](index.md#plugin-hooks)** | `ctx.register_hook()` in a [plugin](../plugins/index.md) | CLI + Gateway | Tool interception, metrics, guardrails |
| **[Shell hooks](index.md#shell-hooks)** | `hooks:` block in `~/.hermes/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |

All three systems are non-blocking — errors in any hook are caught and logged, never crashing the agent.

## Gateway Event Hooks[​](index.md#gateway-event-hooks "Direct link to Gateway Event Hooks")
...
### Creating a Hook[​](index.md#creating-a-hook "Direct link to Creating a Hook")
...
#### HOOK.yaml[​](index.md#hookyaml "Direct link to HOOK.yaml")
...
#### handler.py[​](index.md#handlerpy "Direct link to handler.py")
...
### Available Events[​](index.md#available-events "Direct link to Available Events")
...
#### Wildcard Matching[​](index.md#wildcard-matching "Direct link to Wildcard Matching")

Handlers registered for `command:*` fire for any `command:` event (`command:model`, `command:reset`, etc.). Monitor all slash commands with a single subscription.

### Examples[​](index.md#examples "Direct link to Examples")

#### Telegram Alert on Long Tasks[​](index.md#telegram-alert-on-long-tasks "Direct link to Telegram Alert on Long Tasks")
...
#### Command Usage Logger[​](index.md#command-usage-logger "Direct link to Command Usage Logger")
...
#### Session Start Webhook[​](index.md#session-start-webhook "Direct link to Session Start Webhook")
...
### Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot[​](index.md#tutorial-bootmd--run-a-startup-checklist-on-every-gateway-boot "Direct link to Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot")
...
#### Step 1: Write your checklist[​](index.md#step-1-write-your-checklist "Direct link to Step 1: Write your checklist")
...
#### Step 2: Create the hook[​](index.md#step-2-create-the-hook "Direct link to Step 2: Create the hook")
...
#### Step 3: Test it[​](index.md#step-3-test-it "Direct link to Step 3: Test it")
...
#### Extending the pattern[​](index.md#extending-the-pattern "Direct link to Extending the pattern")
...
#### Why this isn't a built-in[​](index.md#why-this-isnt-a-built-in "Direct link to Why this isn't a built-in")
...
Gateway hooks only fire in the **gateway** (Telegram, Discord, Slack, WhatsApp, Teams). The CLI does not load gateway hooks. For hooks that work everywhere, use [plugin hooks](index.md#plugin-hooks).

## Plugin Hooks[​](index.md#plugin-hooks "Direct link to Plugin Hooks")

[Plugins](../plugins/index.md) can register hooks that fire in **both CLI and gateway** sessions. These are registered programmatically via `ctx.register_hook()` in your plugin's `register()` function.
...
- Two hooks' return values affect behavior: [`pre_tool_call`](index.md#pre_tool_call) can **block** the tool, and [`pre_llm_call`](index.md#pre_llm_call) can **inject context** into the LLM call. All other hooks are fire-and-forget observers.

### Quick reference[​](index.md#quick-reference "Direct link to Quick reference")

| Hook | Fires when | Returns |
| --- | --- | --- |
| [`pre_tool_call`](index.md#pre_tool_call) | Before any tool executes | `{"action": "block", "message": str}` to veto the call |
| [`post_tool_call`](index.md#post_tool_call) | After any tool returns | ignored |
| [`pre_llm_call`](index.md#pre_llm_call) | Once per turn, before the tool-calling loop | `{"context": str}` to prepend context to the user message |
| [`post_llm_call`](index.md#post_llm_call) | Once per turn, after the tool-calling loop | ignored |
| [`on_session_start`](index.md#on_session_start) | New session created (first turn only) | ignored |
| [`on_session_end`](index.md#on_session_end) | Session ends | ignored |
| [`on_session_finalize`](index.md#on_session_finalize) | CLI/gateway tears down an active session (flush, save, stats) | ignored |
| [`on_session_reset`](index.md#on_session_reset) | Gateway swaps in a fresh session key (e.g. `/new`, `/reset`) | ignored |
| [`subagent_stop`](index.md#subagent_stop) | A `delegate_task` child has exited | ignored |
| [`pre_gateway_dispatch`](index.md#pre_gateway_dispatch) | Gateway received a user message, before auth + dispatch | `{"action": "skip" | "rewrite" | "allow", ...}` to influence flow |
| [`pre_approval_request`](index.md#pre_approval_request) | Dangerous command needs user approval, before the prompt/notification is sent | ignored |
| [`post_approval_response`](index.md#post_approval_response) | User responded to an approval prompt (or it timed out) | ignored |
| [`transform_tool_result`](index.md#transform_tool_result) | After any tool returns, before the result is handed back to the model | `str` to replace the result, `None` to leave unchanged |
| [`transform_terminal_output`](index.md#transform_terminal_output) | Inside the `terminal` tool, before truncation/ANSI-strip/redact | `str` to replace the raw output, `None` to leave unchanged |
| [`transform_llm_output`](index.md#transform_llm_output) | After the tool-calling loop completes, before the final response is delivered | `str` to replace the response text, `None`/empty to leave unchanged |

---

### `pre_tool_call`[​](index.md#pre_tool_call "Direct link to pre_tool_call")
...
### `post_tool_call`[​](index.md#post_tool_call "Direct link to post_tool_call")
...
### `pre_llm_call`[​](index.md#pre_llm_call "Direct link to pre_llm_call")
...
### `post_llm_call`[​](index.md#post_llm_call "Direct link to post_llm_call")
...
### `on_session_start`[​](index.md#on_session_start "Direct link to on_session_start")
...
### `on_session_end`[​](index.md#on_session_end "Direct link to on_session_end")
...
### `on_session_finalize`[​](index.md#on_session_finalize "Direct link to on_session_finalize")
...
### `on_session_reset`[​](index.md#on_session_reset "Direct link to on_session_reset")
...
See the **[Build a Plugin guide](../../../guides/build-a-hermes-plugin/index.md)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.

---

### `subagent_stop`[​](index.md#subagent_stop "Direct link to subagent_stop")
...
### `pre_gateway_dispatch`[​](index.md#pre_gateway_dispatch "Direct link to pre_gateway_dispatch")
...
### `pre_approval_request`[​](index.md#pre_approval_request "Direct link to pre_approval_request")
...
**Return value:** ignored. Hooks here are observer-only; they cannot veto or pre-answer the approval. Use [`pre_tool_call`](index.md#pre_tool_call) to block a tool before it reaches the approval system.
...
### `post_approval_response`[​](index.md#post_approval_response "Direct link to post_approval_response")
...
### `transform_tool_result`[​](index.md#transform_tool_result "Direct link to transform_tool_result")
...
### `transform_terminal_output`[​](index.md#transform_terminal_output "Direct link to transform_terminal_output")
...
### `transform_llm_output`[​](index.md#transform_llm_output "Direct link to transform_llm_output")
...
## Shell Hooks[​](index.md#shell-hooks "Direct link to Shell Hooks")
...
- **Inject context into the next LLM turn** — prepend `git status` output, the current weekday, or retrieved documents to the user message (see [`pre_llm_call`](index.md#pre_llm_call)).
...
### Comparison at a glance[​](index.md#comparison-at-a-glance "Direct link to Comparison at a glance")

| Dimension | Shell hooks | [Plugin hooks](index.md#plugin-hooks) | [Gateway hooks](index.md#gateway-event-hooks) |
| --- | --- | --- | --- |
...
### Configuration schema[​](index.md#configuration-schema "Direct link to Configuration schema")
...
Event names must be one of the [plugin hook events](index.md#plugin-hooks); typos produce a "Did you mean X?" warning and are skipped. Unknown keys inside a single entry are ignored; missing `command` is a skip-with-warning. `timeout > 300` is clamped with a warning.

### JSON wire protocol[​](index.md#json-wire-protocol "Direct link to JSON wire protocol")
...
### Worked examples[​](index.md#worked-examples "Direct link to Worked examples")

#### 1. Auto-format Python files after every write[​](index.md#1-auto-format-python-files-after-every-write "Direct link to 1. Auto-format Python files after every write")
...
#### 2. Block destructive `terminal` commands[​](index.md#2-block-destructive-terminal-commands "Direct link to 2-block-destructive-terminal-commands")
...
#### 3. Inject `git status` into every turn (Claude-Code `UserPromptSubmit` equivalent)[​](index.md#3-inject-git-status-into-every-turn-claude-code-userpromptsubmit-equivalent "Direct link to 3-inject-git-status-into-every-turn-claude-code-userpromptsubmit-equivalent")
...
#### 4. Log every subagent completion[​](index.md#4-log-every-subagent-completion "Direct link to 4. Log every subagent completion")
...
### Consent model[​](index.md#consent-model "Direct link to Consent model")
...
### The `hermes hooks` CLI[​](index.md#the-hermes-hooks-cli "Direct link to the-hermes-hooks-cli")
...
### Ordering and precedence[​](index.md#ordering-and-precedence "Direct link to Ordering and precedence")
...
- [Gateway Event Hooks](index.md#gateway-event-hooks)
  - [Creating a Hook](index.md#creating-a-hook)
  - [Available Events](index.md#available-events)
  - [Examples](index.md#examples)
  - [Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot](index.md#tutorial-bootmd--run-a-startup-checklist-on-every-gateway-boot)
  - [How It Works](index.md#how-it-works)
- [Plugin Hooks](index.md#plugin-hooks)
  - [Quick reference](index.md#quick-reference)
  - [`pre_tool_call`](index.md#pre_tool_call)
  - [`post_tool_call`](index.md#post_tool_call)
  - [`pre_llm_call`](index.md#pre_llm_call)
  - [`post_llm_call`](index.md#post_llm_call)
  - [`on_session_start`](index.md#on_session_start)
  - [`on_session_end`](index.md#on_session_end)
  - [`on_session_finalize`](index.md#on_session_finalize)
  - [`on_session_reset`](index.md#on_session_reset)
  - [`subagent_stop`](index.md#subagent_stop)
  - [`pre_gateway_dispatch`](index.md#pre_gateway_dispatch)
  - [`pre_approval_request`](index.md#pre_approval_request)
  - [`post_approval_response`](index.md#post_approval_response)
  - [`transform_tool_result`](index.md#transform_tool_result)
  - [`transform_terminal_output`](index.md#transform_terminal_output)
  - [`transform_llm_output`](index.md#transform_llm_output)
- [Shell Hooks](index.md#shell-hooks)
  - [Comparison at a glance](index.md#comparison-at-a-glance)
  - [Configuration schema](index.md#configuration-schema)
  - [JSON wire protocol](index.md#json-wire-protocol)
  - [Worked examples](index.md#worked-examples)
  - [Consent model](index.md#consent-model)
  - [The `hermes hooks` CLI](index.md#the-hermes-hooks-cli)
  - [Security](index.md#security)
  - [Ordering and precedence](index.md#ordering-and-precedence)
