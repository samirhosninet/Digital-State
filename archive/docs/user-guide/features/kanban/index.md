...
> **Want a walkthrough?** Read the [Kanban tutorial](../kanban-tutorial/index.md) — four user stories (solo dev, fleet farming, role pipeline with retry, circuit breaker) with dashboard screenshots of each. This page is the reference; the tutorial is the narrative.
...
### Two surfaces: the model talks through tools, you talk through the CLI[​](index.md#two-surfaces-the-model-talks-through-tools-you-talk-through-the-cli "Direct link to Two surfaces: the model talks through tools, you talk through the CLI")

The board has two front doors, both backed by the same `~/.hermes/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `hermes kanban`. See [How workers interact with the board](index.md#how-workers-interact-with-the-board) below.
...
## Kanban vs. `delegate_task`[​](index.md#kanban-vs-delegate_task "Direct link to kanban-vs-delegate_task")
...
## Core concepts[​](index.md#core-concepts "Direct link to Core concepts")
...
- **Board** — a standalone queue of tasks with its own SQLite DB, workspaces
  ...
  (e.g. one per project, repo, or domain); see [Boards (multi-project)](index.md#boards-multi-project)
...
## Boards (multi-project)[​](index.md#boards-multi-project "Direct link to Boards (multi-project)")
...
### Managing boards from the CLI[​](index.md#managing-boards-from-the-cli "Direct link to Managing boards from the CLI")
...
### Managing boards from the dashboard[​](index.md#managing-boards-from-the-dashboard "Direct link to Managing boards from the dashboard")
...
The commands below are **you** (the human) setting up the board and creating tasks. Once a task is assigned, the dispatcher spawns the assigned profile as a worker, and from there **the model drives the task through `kanban_*` tool calls, not CLI commands** — see [How workers interact with the board](index.md#how-workers-interact-with-the-board).
...
### Gateway-embedded dispatcher (default)[​](index.md#gateway-embedded-dispatcher-default "Direct link to Gateway-embedded dispatcher (default)")
...
### Idempotent create (for automation / webhooks)[​](index.md#idempotent-create-for-automation--webhooks "Direct link to Idempotent create (for automation / webhooks)")
...
### Bulk CLI verbs[​](index.md#bulk-cli-verbs "Direct link to Bulk CLI verbs")
...
## How workers interact with the board[​](index.md#how-workers-interact-with-the-board "Direct link to How workers interact with the board")
...
### Why tools instead of shelling to `hermes kanban`[​](index.md#why-tools-instead-of-shelling-to-hermes-kanban "Direct link to why-tools-instead-of-shelling-to-hermes-kanban")
...
### Recommended handoff evidence[​](index.md#recommended-handoff-evidence "Direct link to Recommended handoff evidence")
...
### The worker skill[​](index.md#the-worker-skill "Direct link to The worker skill")
...
### Pinning extra skills to a specific task[​](index.md#pinning-extra-skills-to-a-specific-task "Direct link to Pinning extra skills to a specific task")
...
### The orchestrator skill[​](index.md#the-orchestrator-skill "Direct link to The orchestrator skill")
...
### What the plugin gives you[​](index.md#what-the-plugin-gives-you "Direct link to What the plugin gives you")
...
### Architecture[​](index.md#architecture "Direct link to Architecture")

The GUI is strictly a **read-through-the-DB + write-through-kanban\_db** layer with no domain logic of its own:
...
### REST surface[​](index.md#rest-surface "Direct link to REST surface")
...
### Dashboard config[​](index.md#dashboard-config "Direct link to Dashboard config")
...
The dashboard's HTTP auth middleware [explicitly skips `/api/plugins/`](../extending-the-dashboard/index.md#backend-api-routes) — plugin routes are unauthenticated by design because the dashboard binds to localhost by default. That means the kanban REST surface is reachable from any process on the host.
...
### Live updates[​](index.md#live-updates "Direct link to Live updates")
...
### Extending it[​](index.md#extending-it "Direct link to Extending it")

The plugin uses the standard Hermes dashboard plugin contract — see [Extending the Dashboard](../extending-the-dashboard/index.md) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
### Scope boundary[​](index.md#scope-boundary "Direct link to Scope boundary")
...
## CLI command reference[​](index.md#cli-command-reference "Direct link to CLI command reference")

This is the surface **you** (or scripts, cron, the dashboard) use to drive the board. Workers running inside the dispatcher use the `kanban_*` [tool surface](index.md#how-workers-interact-with-the-board) for the same operations — the CLI here and the tools there both route through `kanban_db`, so the two surfaces agree by construction.
...
All commands are also available as a slash command in the interactive CLI and in the messaging gateway (see [`/kanban` slash command](index.md#kanban-slash-command) below).

## `/kanban` slash command[​](index.md#kanban-slash-command "Direct link to kanban-slash-command")
...
### Mid-run usage: `/kanban` bypasses the running-agent guard[​](index.md#mid-run-usage-kanban-bypasses-the-running-agent-guard "Direct link to mid-run-usage-kanban-bypasses-the-running-agent-guard")
...
### Auto-subscribe on `/kanban create` (gateway only)[​](index.md#auto-subscribe-on-kanban-create-gateway-only "Direct link to auto-subscribe-on-kanban-create-gateway-only")
...
### Output truncation in messaging[​](index.md#output-truncation-in-messaging "Direct link to Output truncation in messaging")
...
### Autocomplete[​](index.md#autocomplete "Direct link to Autocomplete")
...
## Collaboration patterns[​](index.md#collaboration-patterns "Direct link to Collaboration patterns")
...
## Multi-tenant usage[​](index.md#multi-tenant-usage "Direct link to Multi-tenant usage")
...
## Gateway notifications[​](index.md#gateway-notifications "Direct link to Gateway notifications")
...
## Runs — one row per attempt[​](index.md#runs--one-row-per-attempt "Direct link to Runs — one row per attempt")
...
### Forward compatibility[​](index.md#forward-compatibility "Direct link to Forward compatibility")
...
## Event reference[​](index.md#event-reference "Direct link to Event reference")
...
## Out of scope[​](index.md#out-of-scope "Direct link to Out of scope")
...
## Design spec[​](index.md#design-spec "Direct link to Design spec")
...
- [Two surfaces: the model talks through tools, you talk through the CLI](index.md#two-surfaces-the-model-talks-through-tools-you-talk-through-the-cli)
- [Kanban vs. `delegate_task`](index.md#kanban-vs-delegate_task)
- [Core concepts](index.md#core-concepts)
- [Boards (multi-project)](index.md#boards-multi-project)
  - [Managing boards from the CLI](index.md#managing-boards-from-the-cli)
  - [Managing boards from the dashboard](index.md#managing-boards-from-the-dashboard)
- [Quick start](index.md#quick-start)
  - [Gateway-embedded dispatcher (default)](index.md#gateway-embedded-dispatcher-default)
  - [Idempotent create (for automation / webhooks)](index.md#idempotent-create-for-automation--webhooks)
  - [Bulk CLI verbs](index.md#bulk-cli-verbs)
- [How workers interact with the board](index.md#how-workers-interact-with-the-board)
  - [Why tools instead of shelling to `hermes kanban`](index.md#why-tools-instead-of-shelling-to-hermes-kanban)
  - [Recommended handoff evidence](index.md#recommended-handoff-evidence)
  - [The worker skill](index.md#the-worker-skill)
  - [Pinning extra skills to a specific task](index.md#pinning-extra-skills-to-a-specific-task)
  - [The orchestrator skill](index.md#the-orchestrator-skill)
- [Dashboard (GUI)](index.md#dashboard-gui)
  - [What the plugin gives you](index.md#what-the-plugin-gives-you)
  - [Architecture](index.md#architecture)
  - [REST surface](index.md#rest-surface)
  - [Dashboard config](index.md#dashboard-config)
  - [Security model](index.md#security-model)
  - [Live updates](index.md#live-updates)
  - [Extending it](index.md#extending-it)
  - [Scope boundary](index.md#scope-boundary)
- [CLI command reference](index.md#cli-command-reference)
- [`/kanban` slash command](index.md#kanban-slash-command)
  - [Mid-run usage: `/kanban` bypasses the running-agent guard](index.md#mid-run-usage-kanban-bypasses-the-running-agent-guard)
  - [Auto-subscribe on `/kanban create` (gateway only)](index.md#auto-subscribe-on-kanban-create-gateway-only)
  - [Output truncation in messaging](index.md#output-truncation-in-messaging)
  - [Autocomplete](index.md#autocomplete)
- [Collaboration patterns](index.md#collaboration-patterns)
- [Multi-tenant usage](index.md#multi-tenant-usage)
- [Gateway notifications](index.md#gateway-notifications)
- [Runs — one row per attempt](index.md#runs--one-row-per-attempt)
  - [Forward compatibility](index.md#forward-compatibility)
- [Event reference](index.md#event-reference)
- [Out of scope](index.md#out-of-scope)
- [Design spec](index.md#design-spec)
