...
2. **Docker as a terminal backend** — the agent runs on your host but executes every command inside a single, persistent Docker sandbox container that survives across tool calls, `/new`, and subagents for the life of the Hermes process (see [Configuration → Docker Backend](../configuration/index.md#docker-backend))
...
## Running in gateway mode[​](index.md#running-in-gateway-mode "Direct link to Running in gateway mode")
...
Port 8642 exposes the gateway's [OpenAI-compatible API server](../features/api-server/index.md) and health endpoint. It's optional if you only use chat platforms (Telegram, Discord, etc.), but required if you want the dashboard or external tools to reach the gateway.
...
## Running the dashboard[​](index.md#running-the-dashboard "Direct link to Running the dashboard")
...
## Running interactively (CLI chat)[​](index.md#running-interactively-cli-chat "Direct link to Running interactively (CLI chat)")
...
## Persistent volumes[​](index.md#persistent-volumes "Direct link to Persistent volumes")
...
## Multi-profile support[​](index.md#multi-profile-support "Direct link to Multi-profile support")

Hermes supports [multiple profiles](../../reference/profile-commands/index.md) — separate `~/.hermes/` directories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **When running under Docker, using Hermes' built-in multi-profile feature is not recommended.**
...
## Environment variable forwarding[​](index.md#environment-variable-forwarding "Direct link to Environment variable forwarding")
...
## Docker Compose example[​](index.md#docker-compose-example "Direct link to Docker Compose example")
...
## What the Dockerfile does[​](index.md#what-the-dockerfile-does "Direct link to What the Dockerfile does")
...
- **`openssh-client`** — enables the [SSH terminal backend](../configuration/index.md#ssh-backend) from inside the container. The SSH backend shells out to the system `ssh` binary; without this, it failed silently in containerized installs.
...
- Optionally launches `hermes dashboard` as a background side-process when `HERMES_DASHBOARD=1` (see [Running the dashboard](index.md#running-the-dashboard))
...
## Upgrading[​](index.md#upgrading "Direct link to Upgrading")
...
## Skills and credential files[​](index.md#skills-and-credential-files "Direct link to Skills and credential files")

When using Docker as the execution environment (not the methods above, but when the agent runs commands inside a Docker sandbox — see [Configuration → Docker Backend](../configuration/index.md#docker-backend)), Hermes reuses a single long-lived container for all tool calls and automatically bind-mounts the skills directory (`~/.hermes/skills/`) and any credential files declared by skills into that container as read-only volumes. Skill scripts, templates, and references are available inside the sandbox without manual configuration, and because the container persists for the life of the Hermes process, any dependencies you install or files you write stay around for the next tool call.

The same syncing happens for SSH and Modal backends — skills and credential files are uploaded via rsync or the Modal mount API before each command.

## Connecting to local inference servers (vLLM, Ollama, etc.)[​](index.md#connecting-to-local-inference-servers-vllm-ollama-etc "Direct link to Connecting to local inference servers (vLLM, Ollama, etc.)")
...
### Docker Compose (recommended)[​](index.md#docker-compose-recommended "Direct link to Docker Compose (recommended)")
...
### Standalone Docker run (no Compose)[​](index.md#standalone-docker-run-no-compose "Direct link to Standalone Docker run (no Compose)")
...
### Verifying connectivity[​](index.md#verifying-connectivity "Direct link to Verifying connectivity")
...
### Ollama[​](index.md#ollama "Direct link to Ollama")
...
### Container exits immediately[​](index.md#container-exits-immediately "Direct link to Container exits immediately")
...
### "Permission denied" errors[​](index.md#permission-denied-errors "Direct link to \"Permission denied\" errors")
...
### Browser tools not working[​](index.md#browser-tools-not-working "Direct link to Browser tools not working")
...
### Gateway not reconnecting after network issues[​](index.md#gateway-not-reconnecting-after-network-issues "Direct link to Gateway not reconnecting after network issues")
...
### Checking container health[​](index.md#checking-container-health "Direct link to Checking container health")
...
- [Running in gateway mode](index.md#running-in-gateway-mode)
- [Running the dashboard](index.md#running-the-dashboard)
- [Running interactively (CLI chat)](index.md#running-interactively-cli-chat)
- [Persistent volumes](index.md#persistent-volumes)
- [Multi-profile support](index.md#multi-profile-support)
- [Environment variable forwarding](index.md#environment-variable-forwarding)
- [Docker Compose example](index.md#docker-compose-example)
- [Resource limits](index.md#resource-limits)
- [What the Dockerfile does](index.md#what-the-dockerfile-does)
- [Upgrading](index.md#upgrading)
- [Skills and credential files](index.md#skills-and-credential-files)
- [Connecting to local inference servers (vLLM, Ollama, etc.)](index.md#connecting-to-local-inference-servers-vllm-ollama-etc)
  - [Docker Compose (recommended)](index.md#docker-compose-recommended)
  - [Standalone Docker run (no Compose)](index.md#standalone-docker-run-no-compose)
  - [Verifying connectivity](index.md#verifying-connectivity)
  - [Ollama](index.md#ollama)
- [Troubleshooting](index.md#troubleshooting)
  - [Container exits immediately](index.md#container-exits-immediately)
  - ["Permission denied" errors](index.md#permission-denied-errors)
  - [Browser tools not working](index.md#browser-tools-not-working)
  - [Gateway not reconnecting after network issues](index.md#gateway-not-reconnecting-after-network-issues)
  - [Checking container health](index.md#checking-container-health)
