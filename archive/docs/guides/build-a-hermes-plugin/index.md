...
title: Build a Hermes Plugin | Hermes Agent
...
| An **LLM / inference backend** (new provider) | [Model Provider Plugins](../../developer-guide/model-provider-plugin/index.md) |
| A **gateway channel** (Discord/Telegram/IRC/Teams/etc.) | [Adding Platform Adapters](../../developer-guide/adding-platform-adapters/index.md) |
| A **memory backend** (Honcho/Mem0/Supermemory/etc.) | [Memory Provider Plugins](../../developer-guide/memory-provider-plugin/index.md) |
| A **context-compression engine** | [Context Engine Plugins](../../developer-guide/context-engine-plugin/index.md) |
| An **image-generation backend** | [Image Generation Provider Plugins](../../developer-guide/image-gen-provider-plugin/index.md) |
| A **TTS backend** (any CLI — Piper, VoxCPM, Kokoro, voice cloning, …) | [TTS custom command providers](../../user-guide/features/tts/index.md#custom-command-providers) — config-driven, no Python needed |
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](../../user-guide/features/tts/index.md#voice-message-transcription-stt) — set `HERMES_LOCAL_STT_COMMAND` to a shell template |
| **External tools via MCP** (filesystem, GitHub, Linear, any MCP server) | [MCP](../../user-guide/features/mcp/index.md) — declare `mcp_servers.<name>` in `config.yaml` |
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](../../user-guide/features/hooks/index.md#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.hermes/hooks/<name>/` |
| **Shell hooks** (run a shell command on events) | [Shell Hooks](../../user-guide/features/hooks/index.md#shell-hooks) — declare under `hooks:` in `config.yaml` |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](../../user-guide/features/skills/index.md) — `hermes skills tap add <repo>` · [Publishing a tap](../../user-guide/features/skills/index.md#publishing-a-custom-skill-tap) |
| A first-class **core** inference provider (not a plugin) | [Adding Providers](../../developer-guide/adding-providers/index.md) |

See the full [Pluggable interfaces table](../../user-guide/features/plugins/index.md#pluggable-interfaces--where-to-go-for-each) for a consolidated view of every extension surface including config-driven (TTS, STT, MCP, shell hooks) and drop-in directory (gateway hooks) styles.

## What you're building[​](index.md#what-youre-building "Direct link to What you're building")
...
## Step 1: Create the plugin directory[​](index.md#step-1-create-the-plugin-directory "Direct link to Step 1: Create the plugin directory")
...
## Step 2: Write the manifest[​](index.md#step-2-write-the-manifest "Direct link to Step 2: Write the manifest")
...
## Step 3: Write the tool schemas[​](index.md#step-3-write-the-tool-schemas "Direct link to Step 3: Write the tool schemas")
...
## Step 4: Write the tool handlers[​](index.md#step-4-write-the-tool-handlers "Direct link to Step 4: Write the tool handlers")
...
## Step 5: Write the registration[​](index.md#step-5-write-the-registration "Direct link to Step 5: Write the registration")
...
- `ctx.register_command()` registers an in-session slash command (e.g. `/myplugin <args>` inside CLI / gateway chat) — see [Register slash commands](index.md#register-slash-commands) below
- `ctx.dispatch_tool(name, arguments)` — call any other tool (built-in or from another plugin) with the parent agent's context (approvals, credentials, task\_id) wired up automatically. Useful from slash-command handlers that need to invoke `terminal`, `read_file`, or any other tool as if the model had called it directly.
...
## Step 6: Test it[​](index.md#step-6-test-it "Direct link to Step 6: Test it")
...
### Debugging plugin discovery[​](index.md#debugging-plugin-discovery "Direct link to Debugging plugin discovery")
...
## Your plugin's final structure[​](index.md#your-plugins-final-structure "Direct link to Your plugin's final structure")
...
## What else can plugins do?[​](index.md#what-else-can-plugins-do "Direct link to What else can plugins do?")

### Ship data files[​](index.md#ship-data-files "Direct link to Ship data files")
...
### Bundle skills[​](index.md#bundle-skills "Direct link to Bundle skills")
...
### Gate on environment variables[​](index.md#gate-on-environment-variables "Direct link to Gate on environment variables")
...
If `WEATHER_API_KEY` isn't set, the plugin is disabled with a clear message. No crash, no error in the agent — just "Plugin weather disabled (missing: WEATHER\_API\_KEY)".
...
### Conditional tool availability[​](index.md#conditional-tool-availability "Direct link to Conditional tool availability")
...
### Register multiple hooks[​](index.md#register-multiple-hooks "Direct link to Register multiple hooks")
...
### Hook reference[​](index.md#hook-reference "Direct link to Hook reference")

Each hook is documented in full on the **[Event Hooks reference](../../user-guide/features/hooks/index.md#plugin-hooks)** — callback signatures, parameter tables, exactly when each fires, and examples. Here's the summary:

| Hook | Fires when | Callback signature | Returns |
| --- | --- | --- | --- |
| [`pre_tool_call`](../../user-guide/features/hooks/index.md#pre_tool_call) | Before any tool executes | `tool_name: str, args: dict, task_id: str` | ignored |
| [`post_tool_call`](../../user-guide/features/hooks/index.md#post_tool_call) | After any tool returns | `tool_name: str, args: dict, result: str, task_id: str, duration_ms: int` | ignored |
| [`pre_llm_call`](../../user-guide/features/hooks/index.md#pre_llm_call) | Once per turn, before the tool-calling loop | `session_id: str, user_message: str, conversation_history: list, is_first_turn: bool, model: str, platform: str` | [context injection](index.md#pre_llm_call-context-injection) |
| [`post_llm_call`](../../user-guide/features/hooks/index.md#post_llm_call) | Once per turn, after the tool-calling loop (successful turns only) | `session_id: str, user_message: str, assistant_response: str, conversation_history: list, model: str, platform: str` | ignored |
| [`on_session_start`](../../user-guide/features/hooks/index.md#on_session_start) | New session created (first turn only) | `session_id: str, model: str, platform: str` | ignored |
| [`on_session_end`](../../user-guide/features/hooks/index.md#on_session_end) | End of every `run_conversation` call + CLI exit | `session_id: str, completed: bool, interrupted: bool, model: str, platform: str` | ignored |
| [`on_session_finalize`](../../user-guide/features/hooks/index.md#on_session_finalize) | CLI/gateway tears down an active session | `session_id: str | None, platform: str` | ignored |
| [`on_session_reset`](../../user-guide/features/hooks/index.md#on_session_reset) | Gateway swaps in a new session key (`/new`, `/reset`) | `session_id: str, platform: str` | ignored |

Most hooks are fire-and-forget observers — their return values are ignored. The exception is `pre_llm_call`, which can inject context into the conversation.

All callbacks should accept `**kwargs` for forward compatibility. If a hook callback crashes, it's logged and skipped. Other hooks and the agent continue normally.

### `pre_llm_call` context injection[​](index.md#pre_llm_call-context-injection "Direct link to pre_llm_call-context-injection")

This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Hermes injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format[​](index.md#return-format "Direct link to Return format")
...
#### How injection works[​](index.md#how-injection-works "Direct link to How injection works")
...
#### Example: Memory recall plugin[​](index.md#example-memory-recall-plugin "Direct link to Example: Memory recall plugin")
...
#### Example: Guardrails plugin[​](index.md#example-guardrails-plugin "Direct link to Example: Guardrails plugin")
...
#### Example: Observer-only hook (no injection)[​](index.md#example-observer-only-hook-no-injection "Direct link to Example: Observer-only hook (no injection)")
...
#### Multiple plugins returning context[​](index.md#multiple-plugins-returning-context "Direct link to Multiple plugins returning context")
...
### Register CLI commands[​](index.md#register-cli-commands "Direct link to Register CLI commands")
...
**Memory provider plugins** use a convention-based approach instead: add a `register_cli(subparser)` function to your plugin's `cli.py` file. The memory plugin discovery system finds it automatically — no `ctx.register_cli_command()` call needed. See the [Memory Provider Plugin guide](../../developer-guide/memory-provider-plugin/index.md#adding-cli-commands) for details.
...
### Register slash commands[​](index.md#register-slash-commands "Direct link to Register slash commands")
...
### Dispatch tools from slash commands[​](index.md#dispatch-tools-from-slash-commands "Direct link to Dispatch tools from slash commands")
...
## Specialized plugin types[​](index.md#specialized-plugin-types "Direct link to Specialized plugin types")
...
### Model provider plugins — add an LLM backend[​](index.md#model-provider-plugins--add-an-llm-backend "Direct link to Model provider plugins — add an LLM backend")
...
Lazy-discovered the first time anything calls `get_provider_profile()` or `list_providers()` — `auth.py`, `config.py`, `doctor.py`, `models.py`, `runtime_provider.py`, and the chat\_completions transport auto-wire to it. User plugins override bundled ones by name.

**Full guide:** [Model Provider Plugins](../../developer-guide/model-provider-plugin/index.md) — field reference, overridable hooks (`prepare_messages`, `build_extra_body`, `build_api_kwargs_extras`, `fetch_models`), api\_mode selection, auth types, testing.

### Platform plugins — add a gateway channel[​](index.md#platform-plugins--add-a-gateway-channel "Direct link to Platform plugins — add a gateway channel")
...
**Full guide:** [Adding Platform Adapters](../../developer-guide/adding-platform-adapters/index.md) — complete `BasePlatformAdapter` contract, message routing, auth gating, setup wizard integration. Look at `plugins/platforms/irc/` for a stdlib-only working example.

### Memory provider plugins — add a cross-session knowledge backend[​](index.md#memory-provider-plugins--add-a-cross-session-knowledge-backend "Direct link to Memory provider plugins — add a cross-session knowledge backend")
...
**Full guide:** [Memory Provider Plugins](../../developer-guide/memory-provider-plugin/index.md) — full `MemoryProvider` ABC, threading contract, profile isolation, CLI command registration via `cli.py`.

### Context engine plugins — replace the context compressor[​](index.md#context-engine-plugins--replace-the-context-compressor "Direct link to Context engine plugins — replace the context compressor")
...
**Full guide:** [Context Engine Plugins](../../developer-guide/context-engine-plugin/index.md).

### Image-generation backends[​](index.md#image-generation-backends "Direct link to Image-generation backends")
...
**Full guide:** [Image Generation Provider Plugins](../../developer-guide/image-gen-provider-plugin/index.md) — full `ImageGenProvider` ABC, `list_models()` / `get_setup_schema()` metadata, `success_response()`/`error_response()` helpers, base64 vs URL output, user overrides, pip distribution.
...
## Non-Python extension surfaces[​](index.md#non-python-extension-surfaces "Direct link to Non-Python extension surfaces")

Hermes also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](../../user-guide/features/plugins/index.md#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools[​](index.md#mcp-servers--register-external-tools "Direct link to MCP servers — register external tools")
...
Hermes connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](../../user-guide/features/mcp/index.md).

### Gateway event hooks — fire on lifecycle events[​](index.md#gateway-event-hooks--fire-on-lifecycle-events "Direct link to Gateway event hooks — fire on lifecycle events")
...
**Full guide:** [Gateway Event Hooks](../../user-guide/features/hooks/index.md#gateway-event-hooks).

### Shell hooks — run a shell command on tool calls[​](index.md#shell-hooks--run-a-shell-command-on-tool-calls "Direct link to Shell hooks — run a shell command on tool calls")
...
**Full guide:** [Shell Hooks](../../user-guide/features/hooks/index.md#shell-hooks).

### Skill sources — add a custom skill registry[​](index.md#skill-sources--add-a-custom-skill-registry "Direct link to Skill sources — add a custom skill registry")
...
**Full guides:** [Skills Hub](../../user-guide/features/skills/index.md#skills-hub) · [Publishing a custom tap](../../user-guide/features/skills/index.md#publishing-a-custom-skill-tap) (repo layout, minimal example, non-default paths, trust levels).

### TTS / STT via command templates[​](index.md#tts--stt-via-command-templates "Direct link to TTS / STT via command templates")
...
**Full guides:** [TTS custom command providers](../../user-guide/features/tts/index.md#custom-command-providers) · [STT](../../user-guide/features/tts/index.md#voice-message-transcription-stt).

## Distribute via pip[​](index.md#distribute-via-pip "Direct link to Distribute via pip")
...
## Distribute for NixOS[​](index.md#distribute-for-nixos "Direct link to Distribute for NixOS")
...
See the [Nix Setup guide](../../getting-started/nix-setup/index.md#plugins) for complete documentation including overlay usage and collision checking.

## Common mistakes[​](index.md#common-mistakes "Direct link to Common mistakes")
...
- [What you're building](index.md#what-youre-building)
- [Step 1: Create the plugin directory](index.md#step-1-create-the-plugin-directory)
- [Step 2: Write the manifest](index.md#step-2-write-the-manifest)
- [Step 3: Write the tool schemas](index.md#step-3-write-the-tool-schemas)
- [Step 4: Write the tool handlers](index.md#step-4-write-the-tool-handlers)
- [Step 5: Write the registration](index.md#step-5-write-the-registration)
- [Step 6: Test it](index.md#step-6-test-it)
  - [Debugging plugin discovery](index.md#debugging-plugin-discovery)
- [Your plugin's final structure](index.md#your-plugins-final-structure)
- [What else can plugins do?](index.md#what-else-can-plugins-do)
  - [Ship data files](index.md#ship-data-files)
  - [Bundle skills](index.md#bundle-skills)
  - [Gate on environment variables](index.md#gate-on-environment-variables)
  - [Conditional tool availability](index.md#conditional-tool-availability)
  - [Register multiple hooks](index.md#register-multiple-hooks)
  - [Hook reference](index.md#hook-reference)
  - [`pre_llm_call` context injection](index.md#pre_llm_call-context-injection)
  - [Register CLI commands](index.md#register-cli-commands)
  - [Register slash commands](index.md#register-slash-commands)
  - [Dispatch tools from slash commands](index.md#dispatch-tools-from-slash-commands)
- [Specialized plugin types](index.md#specialized-plugin-types)
  - [Model provider plugins — add an LLM backend](index.md#model-provider-plugins--add-an-llm-backend)
  - [Platform plugins — add a gateway channel](index.md#platform-plugins--add-a-gateway-channel)
  - [Memory provider plugins — add a cross-session knowledge backend](index.md#memory-provider-plugins--add-a-cross-session-knowledge-backend)
  - [Context engine plugins — replace the context compressor](index.md#context-engine-plugins--replace-the-context-compressor)
  - [Image-generation backends](index.md#image-generation-backends)
- [Non-Python extension surfaces](index.md#non-python-extension-surfaces)
  - [MCP servers — register external tools](index.md#mcp-servers--register-external-tools)
  - [Gateway event hooks — fire on lifecycle events](index.md#gateway-event-hooks--fire-on-lifecycle-events)
  - [Shell hooks — run a shell command on tool calls](index.md#shell-hooks--run-a-shell-command-on-tool-calls)
  - [Skill sources — add a custom skill registry](index.md#skill-sources--add-a-custom-skill-registry)
  - [TTS / STT via command templates](index.md#tts--stt-via-command-templates)
- [Distribute via pip](index.md#distribute-via-pip)
- [Distribute for NixOS](index.md#distribute-for-nixos)
- [Common mistakes](index.md#common-mistakes)
